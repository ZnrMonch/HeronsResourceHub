...
public class UsersDatabase extends BaseDatabase {
	...
	private static final String SELECT_USERS_COLS = "user_id, system_role, student_id, first_name, last_name, "
			+ "college, year_level, course_program, karma_score, " + "contact_num, home_address, "
	...
	public List<AdminUsers> getAllUsers() {
		String sql = "SELECT " + SELECT_USERS_COLS + " FROM users WHERE users_is_archived = 0 ORDER BY user_id ASC";
	...
	public List<AdminUsers> getArchivedUsers() {
		String sql = "SELECT " + SELECT_USERS_COLS + " FROM users WHERE users_is_archived = 1 ORDER BY user_id ASC";
		List<AdminUsers> list = new ArrayList<>();
		try (Connection conn = getConn();
				PreparedStatement stmt = conn.prepareStatement(sql);
				ResultSet rs = stmt.executeQuery()) {
	...
	// Shared search path for both active and archived; avoids duplicating two
	...
	// Returns the payment_number decrypted; falls back to raw value for legacy
	// unencrypted rows
	public Map<String, String> getPaymentMethods(int userId) {
		...
		try (Connection conn = getConn(); PreparedStatement stmt = conn.prepareStatement(sql)) {
			...
			while (rs.next()) {
				...
				methods.put(rs.getString("payment_type"), decrypted != null ? decrypted : raw);
			}
		} catch (SQLException e) {
	...
	public int insertUser(String studentId, String firstName, String lastName, String email, String password,
			String yearLevel, String college, String role, String profileImagePath) {
		String sql = "INSERT INTO users " + "(student_id, first_name, last_name, umak_email_address, password, "
	...
	// AES-encrypts the payment number before storage; upserts on (user_id,
	// payment_type)
	...
	// Shared search path for active/archived; avoids duplicating two near-identical
	// methods
	private List<AdminUsers> search(String filter, String keyword, boolean archived) {
		...
		if ("All".equals(filter)) {
			String sql = "SELECT " + SELECT_USERS_COLS + " FROM users WHERE (" + "  CAST(user_id AS CHAR) LIKE ? OR"
					+ "  student_id            LIKE ? OR" + "  first_name            LIKE ? OR"
					+ "  last_name             LIKE ? OR" + "  college               LIKE ? OR"
					+ "  year_level            LIKE ? OR" + "  system_role           LIKE ? OR"
					+ "  contact_num           LIKE ?" + ") AND users_is_archived = " + archiveFlag;
			try (Connection conn = getConn(); PreparedStatement stmt = conn.prepareStatement(sql)) {
				String like = "%" + keyword + "%";
				for (int i = 1; i <= 8; i++)
		...
		String sql = "SELECT " + SELECT_USERS_COLS + " FROM users WHERE " + column + " LIKE ? AND users_is_archived = "
				+ archiveFlag;
		try (Connection conn = getConn(); PreparedStatement stmt = conn.prepareStatement(sql)) {
	...
	// Wraps archive/unarchive in a transaction so user and items are always toggled
	// together
...
//One-time migration — encrypts all existing plain-text rows.
//Call once from AdminPanel, then remove the call.
	public void migratePaymentEncryption() {
		String selectSql = "SELECT payment_id, payment_number FROM payment_methods";
		String updateSql = "UPDATE payment_methods SET payment_number = ? WHERE payment_id = ?";
		...
		try (Connection conn = getConn();
			...
			while (rs.next()) {
				...
				// decrypt() returning non-null means it was already encrypted — skip it
				String decrypted = Encryption.decrypt(raw);
				if (decrypted != null) {
					System.out.println("payment_id " + id + " already encrypted, skipping.");
					continue;
				}

				// Plain text detected — encrypt it now
				String encrypted = Encryption.encrypt(raw);
				if (encrypted == null) {
					System.err.println("payment_id " + id + " encryption failed, skipping.");
					continue;
				}
				...
				try (PreparedStatement update = conn.prepareStatement(updateSql)) {
					...
					System.out.println("Migrated payment_id " + id);
				}
			}
			System.out.println("Payment migration complete.");

		} catch (SQLException e) {
			System.err.println("migratePaymentEncryption() failed: " + e.getMessage());
		}
	}
