...
public class UserFormDialog {
 ...
 public static void showAdd(Window owner, AdminUsersServices userService, Runnable onSuccess) {
     ...
     CustomComboBox<String> yearLevelBox = new CustomComboBox<>(
         new String[]{ "First", "Second", "Third", "Fourth", "Fifth", "Graduate" });
     ...
     profileBrowseBtn.addActionListener(e -> {
         ...
         if (!path.isEmpty()) {
             ...
             profileImageField.setText(path.substring(path.lastIndexOf(java.io.File.separator) + 1));
         }
     });
     ...
     formPanel.add(Box.createVerticalStrut(10));
     formPanel.add(buildManualRow("System Role:",   roleComboPanel));
     formPanel.add(Box.createVerticalStrut(10));
     formPanel.add(buildManualRow("Profile Image:", profileImageInputPanel));
 ...
 // so composite inputs (e.g. role combos, image field + browse button) can size themselves freely
