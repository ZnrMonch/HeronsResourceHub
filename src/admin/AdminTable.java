...
public class AdminTable extends CustomPanel {
	...
	private CustomPanel initHeader() {
		...
		if (type == TableType.USERS)
			...
			titleText = "Item Management";
		else if (type == TableType.LOGS)
			titleText = "Logs Management";
	...
	private String[] getFilterOptions() {
		if (type == TableType.USERS) {
			return new String[] { "ID", "Student ID", "First Name", "Last Name", "College", "Year Level",
					"System Role" };
		} else if (type == TableType.ITEMS) {
	...
	private JPanel buildDateFilterPanel() {
		...
		// Month ComboBox
		String[] months = { "All Months", "Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov",
				"Dec" };
		...
		yearBox.addActionListener(e -> {
			...
			boolean yearChosen = sel != null && !sel.equals("All Years");
		...
		monthBox.addActionListener(e -> {
			...
			boolean monthChosen = selMonth != null && !selMonth.equals("All Months");
			if (monthChosen && selYear != null && !selYear.equals("All Years")) {
		...
		btnDateRange.setDefaultColor(Brand.GREEN);
		...
		btnClearDates.setDefaultColor(Color.decode("#ffc107"));
	...
	private void updateDayCombo(int maxDay) {
		...
		if (currentSel != null && !currentSel.equals("All Days")) {
			try {
				...
				if (prev <= maxDay)
					dayBox.setSelectedItem(currentSel);
				else
					dayBox.setSelectedIndex(0);
	...
	private void openDateRangeDialog() {
		...
		if (activeDateFrom != null) {
			try {
				fromModel.setValue(java.sql.Date.valueOf(activeDateFrom));
			} catch (Exception ignored) {
			}
		}
		if (activeDateTo != null) {
			try {
				toModel.setValue(java.sql.Date.valueOf(activeDateTo));
			} catch (Exception ignored) {
			}
		}
		...
		applyBtn.setDefaultColor(Color.GRAY);
		...
		applyBtn.addActionListener(e -> {
			...
			if (from.after(to)) {
				JOptionPane.showMessageDialog(dialog, "Error! 'From' date cannot be after 'To' date.", "Invalid Range",
	...
	private Object[] getColumnNames() {
		if (type == TableType.USERS) {
			return new Object[] { "ID", "Student ID", "First Name", "Last Name", "College", "Year", "Karma Points",
					"System Role" };
		} else if (type == TableType.ITEMS) {
			return new Object[] { "ID", "Item Name", "Condition", "Category", "Stock", "Price", "Status" };
		} else {
			return new Object[] { "ID", "Log Type", "Description", "Date", "User ID" };
		}
	}
	...
	private void loadTableData() {
		...
		if (type == TableType.USERS) {
			if (archiveMode.isSelected()) {
				data = searching ? userService.searchArchivedUsers(filter, keyword)
						: userService.getArchivedUsersForTable();
			} else {
				data = searching ? userService.searchActiveUsers(filter, keyword)
						: userService.getActiveUsersForTable();
			}

		} else if (type == TableType.ITEMS) {
			if (archiveMode.isSelected()) {
				data = searching ? itemService.searchArchivedItems(filter, keyword)
						: itemService.getArchivedItemsForTable();
			} else {
				data = searching ? itemService.searchActiveItems(filter, keyword)
	...
	private void updateActionUI() {
		if (btnPrimaryAction == null || btnSecondaryAction == null)
			return;
	...
	private void handleUpdate() {
		...
		if (type == TableType.USERS) {
			...
			// In the USERS section of handleUpdate():
			...
			AdminDialog dialog = new AdminDialog(owner, "Update User: " + id, formPanel, "Save Changes", e -> {
				...
				String resolvedFirst = firstName.isEmpty() ? user.getFirstName() : firstName;
				String resolvedLast = lastName.isEmpty() ? user.getLastName() : lastName;
				...
				if (resolvedFirst.isEmpty() || resolvedLast.isEmpty() || resolvedCollege.isEmpty()) {
					JOptionPane.showMessageDialog(this, "Error! First Name, Last Name, and College cannot be empty.",
			...
			// In the ITEMS section of handleUpdate(), update field creation:
			...
			String[] categories = { "Textbooks", "Electronics", "Equipment", "Supplies", "Consumable Goods", "Other" };
			...
			AdminDialog dialog = new AdminDialog(owner, "Update Item: " + id, formPanel, "Save Changes", e -> {
				...
				if (resolvedName.isEmpty() || resolvedCat.isEmpty()) {
					JOptionPane.showMessageDialog(this, "Error: Item Name and Category cannot be empty.",
							"Validation Error", JOptionPane.ERROR_MESSAGE);
					return;
				}
				...
				try {
					...
					JOptionPane.showMessageDialog(this, "Error: Stock and Price must be valid whole numbers.",
							"Validation Error", JOptionPane.ERROR_MESSAGE);
					return;
				}

				if (resolvedStock < 0 || resolvedPrice < 0) {
					JOptionPane.showMessageDialog(this, "Error: Stock and Price cannot be negative.",
							"Validation Error", JOptionPane.ERROR_MESSAGE);
					return;
				}
	...
	// TO DO: NOT YET WORKING PROPERLY
	private void handleArchive() {
		...
		int confirm = JOptionPane.showConfirmDialog(this,
				"Are you sure you want to archive " + typeStr + " " + id + "?", "Confirm Archive",
				JOptionPane.YES_NO_OPTION);

		if (confirm == JOptionPane.YES_OPTION) {
			boolean success = (type == TableType.USERS) ? userService.archiveUser(id) : itemService.archiveItem(id);

			if (success) {
				loadTableData();
				JOptionPane.showMessageDialog(this, "Success! " + typeStr + " archived successfully.");
			} else
				JOptionPane.showMessageDialog(this, "Error: Archive failed. Cannot archive admin account.");
		}
	}
	...
	private void handleRetrieve() {
		...
		String typeStr = type == TableType.USERS ? "user" : "item";
		int confirm = JOptionPane.showConfirmDialog(this,
				"Are you sure you want to restore " + typeStr + " " + id + "?", "Confirm Retrieve",
				JOptionPane.YES_NO_OPTION);

		if (confirm == JOptionPane.YES_OPTION) {
			boolean success = (type == TableType.USERS) ? userService.unarchiveUser(id) : itemService.unarchiveItem(id);

			if (success) {
				loadTableData();
				JOptionPane.showMessageDialog(this, "Success! " + typeStr + " restored successfully.");
			} else
				JOptionPane.showMessageDialog(this, "Error: Restore failed. Please try again.");
		}
	}
	...
	private JPanel createFieldPanel(String label, JComponent component) {
		...
		CustomLabel lbl = new CustomLabel(label, 14f, FontStyle.REGULAR);
		lbl.setPreferredSize(new Dimension(100, 30)); // Fixed width for label
