...
public class AdminTable extends CustomPanel {
 ...
 public AdminTable(TableType type, LogType logType) {
     ...
     // Auto-refresh logs every 30 seconds without user intervention
 ...
 private CustomPanel initTable() {
     ...
     // Checkbox column (col 0) is editable only for Users and Items, not Logs
     ...
     if (type != TableType.LOGS) {
         ...
         // Delegate to archive/retrieve depending on current mode
         ...
         // Clicking column 0 header toggles all checkboxes
 ...
 // ID column index differs between Logs (no checkbox col) and other table types
 ...
 private void applyColumnWidths() {
     if (type == TableType.USERS) {
         ...
         // Description column gets remaining space with a generous minimum
 ...
 private String[] getFilterOptions() {
     if (type == TableType.USERS) {
         ...
         // Item and transaction logs expose an extra Item ID filter
         ...
         protected void done() {
             try {
                 ...
                 if (success) {
                     ...
                     // Surface the service-level error message when available
